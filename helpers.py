...
https://github.com/EthanRosenthal/rec-a-sketch/blob/master/helpers.py.
...
def threshold_interactions_df(df, row_name, col_name, row_min, col_min):
    ...
    while not done:
        ...
        df = df[~df[row_name].isin(col_counts[col_counts < col_min].index
                                   .tolist())]
        row_counts = df.groupby(col_name)[row_name].count()
        df = df[~df[col_name].isin(row_counts[row_counts < row_min].index
                                   .tolist())]
...
def train_test_split(interactions, split_count, fraction=None):
    ...
    if fraction:
        try:
            user_index = np.random.choice(
                ...
                replace=False,
                size=np.int64(np.floor(fraction * train.shape[0]))).tolist()
        except:
    ...
    for user in user_index:
        test_interactions = np.random.choice(
            interactions.getrow(user).indices, size=split_count, replace=False)
...
def print_log(row, header=False, spacing=12):
    ...
    for r in row:
        ...
        if isinstance(r, str):
            ...
            middle += '| {0:^{1}} '.format(r, spacing - 2)
        elif (isinstance(r, float) or isinstance(r, np.float32) or
              isinstance(r, np.float64)):
...
def plot_patk(iterarray, patk, title, k=5):
